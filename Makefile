...
energy-baseline:
	$(COMPOSE_CMD) exec web python energy/baseline_energy.py

energy-db:
	# Start energy tracking (run this in separate terminal for clearer logs) then run k6_db
	$(COMPOSE_CMD) exec -e K6_SERVICE=k6_db web python energy/energy_run.py & \
	$(COMPOSE_CMD) run --rm k6_db

energy-redis:
	$(COMPOSE_CMD) exec -e K6_SERVICE=k6_redis web python energy/energy_run.py & \
	$(COMPOSE_CMD) run --rm k6_redis

energy-compare:
	@echo "Run baseline, then energy-db and energy-redis. Files: k6/results/*.json"
